...
//	// ======================================================
//#if !defined(MARIADB_BASE_VERSION) && !defined(MARIADB_VERSION_ID) && MYSQL_VERSION_ID >= 50710
//	uint32 ssl_mode = SSL_MODE_VERIFY_CA;
//	mysql_options(&self->handle, MYSQL_OPT_SSL_MODE, &ssl_mode);
//#endif
//
//	const char* ssl_ca = "C:\\ssl-certs\\isrgrootx1.pem"; // Caminho absoluto
//	const char* ciphers =
...
//	if (mysql_options(&self->handle, MYSQL_OPT_SSL_CA, ssl_ca) != 0) {
//		ShowSQL("Erro ao definir SSL_CA: %s\n", mysql_error(&self->handle));
//		return SQL_ERROR;
//	}
//
//	// Forçar configuração SSL com cifras explícitas
//	if (mysql_ssl_set(&self->handle, NULL, NULL, ssl_ca, NULL, ciphers) != 0) {
//		ShowSQL("Erro SSL: %s\n", mysql_error(&self->handle));
//		return SQL_ERROR;
//	}
//
...
//	// Tentar conexão
//	// ======================================================
//	if (!mysql_real_connect(&self->handle, host, user, passwd, db, (uint32)port, nullptr, 0)) {
//		ShowSQL("Erro de conexão MySQL: %s\n", mysql_error(&self->handle));
//		return SQL_ERROR;
//	}
//
//	self->keepalive = Sql_P_Keepalive(self);
